...
I have worked in many positions, and have extensive experiences in front-end/back-end developing, Linux system maintain, database maintain, network, security, etc.
...
- Front-end APP: Established the RN react development framework, used popular tools such as Reduc/Reselect/Saga/TypeScript, we used RN to develop two mobile APPs, and one Pad APP. They all packaged using Fastlane.
   - The code architecture has been changed and migrated to TypeScript.
   - I spent most of my time on building the RN APP, fix bugs on various clients, and third-party modules adaptations, there are lots of trivial problems.
...
- Android Pad and PC system customization
   ...
   - I have also tried customizing the Openwrt router system to implement the limits, but this solution was unused.
...
- Build up a team to operation and maintain over hundred PostgreSQL servers, and setup the monitor system.
   - The monitoring system initially used Nagios. After encountering performance issues, using Gearman to expanded to multiple nodes didn’t solve the problem very well, finally I switched it to Zabbix, the passive discovery and passive checking solved the performance issue.
   ...
   - By collecting the queries running in the database to elastic search, developers can use the tool to find slow or frequently queries themselves.
   - I tried to use Pacemaker + Etcd to implement a centralized cluster for PostgreSQL deployment and management, but didn’t finish yet.
- Developed and maintained the CPM and CPC AD delivery system, the page view is about 1 billion per day at the moment.
   ...
   - During the period, I migrated the Ad display engine system based on Apache+Lua to Nginx+lua(Openresty), which improved performance and stability.
